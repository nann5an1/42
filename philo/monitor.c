...
/*   Updated: 2024/11/29 01:03:22 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_if_all_eaten(t_philo *philo)
{
	...
	i = 0;
	done_eating = 1;
	if(philo->program->num_of_times_to_eat == -1 || philo->program->num_of_times_to_eat == 0)
		return (0);
...
{
	t_program *program = (t_program *)args;

	while (1)
	{
		if (check_if_died(program) == 1 || check_if_all_eaten(program->philo) == 1)
			break ;
		else
			continue;
		usleep(1000);
